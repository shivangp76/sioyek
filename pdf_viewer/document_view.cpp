...
void DocumentView::set_virtual_pos(VirtualPos pos, bool is_dragging, bool force) {
    if (!fast_coordinates()) {
        offset = pos;
    }
    else {
        set_offsets(pos.x, pos.y, force, is_dragging);
    }
}
...
float DocumentView::zoom_in_cursor(WindowPos mouse_pos, float zoom_factor) {
    ...
    float new_zoom_level = set_zoom_level(zoom_level * zoom_factor, true,false);
    ...
    set_virtual_pos(new_pos, false, true);
...
std::optional<QString> DocumentView::get_paper_name_under_pos(DocumentPos docpos, bool clean) {
    ...
    if (is_pos_inside_selected_text(docpos)) {
        ...
        if (ref_){
            ...
            if ((reference_info.candidates.size() > 0) && (reference_info.candidates[0].reference_type == ReferenceType::Reference)) {
...
TextUnderPointerInfo DocumentView::find_location_of_text_under_pointer(DocumentPos docpos, int max_size) {
    ...
    if (is_pos_inside_selected_text(docpos)){
        ...
        std::optional<std::wstring> abbr_under_pointer = current_document->get_regex_match_at_position(abbr_regex, flat_chars, docpos, 20, &reference_range);
...
bool DocumentView::is_link_a_reference(const PdfLink& link, const PdfLinkTextInfo& link_info) {
    ...
    if (link_info.chr) {
        ...
        if (reference_info.candidates.size() > 0) {
...
void DocumentView::set_overview_link(PdfLink link) {
    ...
    if (page >= 1) {
        ...
        if (SHOW_REFERENCE_OVERVIEW_HIGHLIGHTS && is_reference){
            ...
            current_candidate.set_highlight_rects(overview_highlight_rects);
        }
...
void DocumentView::focus_rect(DocumentRect rect){
    int selected_index = current_document->get_page_intersecting_rect_index(rect);
...
void DocumentView::focus_on_line_with_index(int page, int index) {
    set_line_index(index, page);
...
bool DocumentView::focus_on_visual_mark_pos(bool moving_down) {
    //float window_x, window_y;

    if (!get_ruler_window_rect().has_value()) {
        return false;
    }

    float thresh = 1 - VISUAL_MARK_NEXT_PAGE_THRESHOLD;
    NormalizedWindowRect ruler_window_rect = get_ruler_window_rect().value();
...
AbsoluteRect DocumentView::move_visual_mark(int offset) {
...
void DocumentView::focus_text(int page, const std::wstring& text) {
    //std::vector<std::wstring> line_texts;
    //std::vector<AbsoluteRect> line_rects;
    const PageMergedLinesInfoAbsolute& line_info = get_document()->get_page_lines(page);
...
void DocumentView::goto_next_block() {
    int ruler_page = get_vertical_line_page();
...
void DocumentView::focus_on_character_offset_into_document(int character_offset_into_document) {
    int page = current_document->get_page_from_character_offset(character_offset_into_document);
    int page_offset = current_document->get_page_offset_into_super_fast_index(page);
    ...
    const std::vector<std::wstring>& page_lines = current_document->get_page_lines(page).merged_line_texts;
    int line_index = 0;
...
void ScratchPad::load(std::wstring scratchpad_file_name){
...
void ScratchPad::save(std::wstring scratchpad_file_name){
    ...
    for (auto& drawing : get_all_drawings()) {
...
std::optional<AbsoluteRect> DocumentView::get_selected_rect_absolute() {
    return get_selected_rectangle();
}

std::optional<DocumentRect> DocumentView::get_selected_rect_document() {
...
void DocumentView::handle_keyboard_select(const std::wstring& text, const std::vector<DocumentRect>& tag_rects) {
    ...
    if (text[0] == '#') {
        ...
        if (parts.size() == 2) {
            ...
            if ((begin_parts.size() == 3) && (end_parts.size() == 3)) {
                ...
                begin_pos = begin_doc_pos.to_absolute(doc());
                end_pos = end_doc_pos.to_absolute(doc());
            }
        }
        ...
        if (parts.size() == 2) {
            ...
            if (srect_.has_value() && erect_.has_value()) {
                ...
                begin_pos_window.x = srect.x0 + 5;
                begin_pos_window.y = (srect.y0 + srect.y1) / 2;
                end_pos_window.x = erect.x0 - 5;
    ...
    if (begin_pos.has_value() && end_pos.has_value()){
        clear_selected_text();
        get_text_selection(
            begin_pos.value(),
            end_pos.value(),
...
std::vector<PagelessDocumentRect> DocumentView::get_current_page_flat_words(std::vector<std::vector<PagelessDocumentRect>>* flat_word_chars) {
    int page = get_current_page_number();
    auto res = doc()->get_page_flat_words(page);
    if (flat_word_chars != nullptr) {
        *flat_word_chars = doc()->get_page_flat_word_chars(page);
    }
    return res;
}
