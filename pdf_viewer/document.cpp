...
void Document::finalize_pdf_document_for_current_file(PdfSpecificsInfo info) {
    pdf_write_options pwo{};
    pdf_write_document(context, info.pdf_doc, info.output, &pwo);
    fz_close_output(context, info.output);
    fz_drop_output(context, info.output);
    ...
    fz_write_buffer(context, file_output, info.buffer);

    fz_drop_buffer(context, info.buffer);
...
void Document::delete_pdf_annotations() {
    PdfSpecificsInfo pdf_info = open_pdf_document_for_current_doc();
    ...
    finalize_pdf_document_for_current_file(pdf_info);
}

void Document::delete_intersecting_annotations(AbsoluteRect rect) {
    DocumentRect doc_rect = rect.to_document(this);
    int page_number = doc_rect.page;
    ...
    while (annot) {
        //page_annotations.push_back(annot);
        fz_rect annot_rect = pdf_bound_annot(context, annot);
        DocumentRect docrect;
        docrect.rect.x0 = annot_rect.x0;
        docrect.rect.x1 = annot_rect.x1;
        docrect.rect.y0 = annot_rect.y0;
        docrect.rect.y1 = annot_rect.y1;
        docrect.page = page_number;

        AbsoluteRect annot_absrect = docrect.to_absolute(this);
        if (annot_absrect.intersects(rect)) {
            page_annotations.push_back(annot);
        }
        annot = pdf_next_annot(context, annot);
    }
...
void Document::embed_single_annot(const std::string& uuid){
    ...
    PdfSpecificsInfo info = open_pdf_document_for_current_doc();
    ...
    if (highlight) {
        auto [highlight_page, highlight_annot] = embed_highlight(info.pdf_doc, page, *highlight);
        pdf_page = highlight_page;
        pdf_annot = highlight_annot;
    }

    if (bookmark) {
        auto [bookmark_page, bookmark_annot] = embed_bookmark(info.pdf_doc, page, *bookmark);
    ...
    finalize_pdf_document_for_current_file(info);
}
...
std::vector<QString> Document::get_page_bib_candidates(int page_number, std::vector<std::vector<PagelessDocumentRect>>* out_rects) {
    ...
    LL_ITER(block, stext_page->first_block) {
        ...
        std::vector<PagelessDocumentRect> block_rects;
        ...
        int refernce_index = block_string.indexOf("References", 0, Qt::CaseInsensitive);
        int bib_index = block_string.indexOf("Bibliography", 0, Qt::CaseInsensitive);

        candidates.push_back(block_string);
        out_rects->push_back(block_rects);


        if (refernce_index == 0 || bib_index == 0) {
            candidates.clear();
            out_rects->clear();
        }
...
std::optional<std::pair<QString, std::vector<PagelessDocumentRect>>> Document::get_page_bib_with_reference(int page_number, std::wstring reference_text) {
    ...
    std::vector<std::vector<PagelessDocumentRect>> bib_rects;
    std::vector<QString> bib_texts_ = get_page_bib_candidates(page_number, &bib_rects);
    std::vector<QString> bib_text_prefixes;
    for (auto bib : bib_texts_) {
        bib_text_prefixes.push_back(bib.left(reference_text.size() + 5));
...
void Document::add_freehand_drawing(FreehandDrawing new_drawing) {
    bool is_uuid_null = true;
    for (int i = 0; i < 16; i++) {
        if (new_drawing.uuid.data[i] != 0) {
            is_uuid_null = false;
        }
    }
    if (is_uuid_null){
        new_drawing.uuid = QUuid::createUuid().toBytes();
    }
...
QMap<int, PageFreehandDrawing> Document::load_drawings_from_io_device(QIODevice& binary_file, bool &is_dirty) const {
    ...
    if (drawing_file_version == 1){
        while (binary_file.bytesAvailable() > 0) {
            ...
            drawing.uuid = QUuid::createUuid().toBytes();
...
std::pair<float, float> Document::get_min_max_annot_x_for_page(int page) {
