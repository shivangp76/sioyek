...
void SioyekRendererBackend::initialize_stuff() {
    for (int i = 0; i < 26; i++) {
...
void SioyekRendererBackend::disable_stencil_for_two_page() {
    return disable_stencil();
}
...
void SioyekRendererBackend::add_coordinates_for_window_point(DocumentView* dv, float window_x, float window_y, float r, int point_polygon_vertices, std::vector<float>& out_coordinates){
...
inline void add_coordinates_for_line_segment(
    const NormalizedWindowPos& p0,
    const NormalizedWindowPos& p1,
    const FreehandDrawingPoint& point0,
    const FreehandDrawingPoint& point1,
    const float width,
    const float height,
    const float thickness_x,
    const float thickness_y,
    const float highlight_factor,
    const float prev_line_x,
    const float prev_line_y,
    std::vector<float>& coordinates){

    float line_direction_x = (p1.x - p0.x) * width;
    float line_direction_y = (p1.y - p0.y) * height;
    ...
    float ortho_x1 = -line_direction_y * thickness_x * point0.thickness * highlight_factor;
    float ortho_y1 = line_direction_x * thickness_y * point0.thickness * highlight_factor;

    float ortho_x2 = -line_direction_y * thickness_x * point1.thickness * highlight_factor;
    float ortho_y2 = line_direction_x * thickness_y * point1.thickness * highlight_factor;

    coordinates.push_back(p0.x - ortho_x1);
    coordinates.push_back(p0.y - ortho_y1);
    coordinates.push_back(p0.x + ortho_x1);
    coordinates.push_back(p0.y + ortho_y1);

    coordinates.push_back(p1.x - ortho_x1);
    coordinates.push_back(p1.y - ortho_y1);
    coordinates.push_back(p1.x + ortho_x2);
...
void PdfViewOpenGLWidget::render_drawings(QPainter* p, DocumentView* dv, const QList<FreehandDrawing>& drawings, bool highlighted) {
    ...
    for (int i = drawings.size() - 1; i >= 0; i--) {
        ...
        for (int line_index = 0; line_index < drawing.points.size() - 1; line_index++) {
            add_coordinates_for_line_segment(
                window_positions[line_index], window_positions[line_index+1],
                drawing.points[line_index], drawing.points[line_index + 1],
                width(), height(),
                thickness_x, thickness_y,
                highlight_factor,
                prev_line_x, prev_line_y,
                coordinates);
        }
...
Qt::ScreenOrientation SioyekRendererBackend::get_orientation() {
    return get_widget()->screen()->orientation();
}

int SioyekRendererBackend::get_width() {
    return get_widget()->width();
}

int SioyekRendererBackend::get_height() {
...
bool SioyekRendererBackend::can_use_cached_scratchpad_framebuffer() {
    auto scratchpad = scratch();
...
float SioyekRendererBackend::get_device_pixel_ratio() {
    return get_widget()->devicePixelRatioF();
}
...
void PdfViewQPainterWidget::render_line_window(float gl_vertical_pos, std::optional<NormalizedWindowRect> ruler_rect) {
...
void PdfViewQPainterWidget::enable_stencil() {
}

void PdfViewQPainterWidget::write_to_stencil() {
}

void PdfViewQPainterWidget::draw_stencil_rects(const std::vector<NormalizedWindowRect>& rects) {
}


void PdfViewQPainterWidget::use_stencil_to_write(bool eq) {
}
...
void PdfViewQPainterWidget::bind_program(ColorPalette forced_palette) {
}

void PdfViewQPainterWidget::bind_points(const std::vector<float>& points) {
}

void PdfViewQPainterWidget::bind_default() {
}

void PdfViewQPainterWidget::bind_vertex_array() {
}

void PdfViewQPainterWidget::prepare_line_drawing_pipeline(){
}

void PdfViewQPainterWidget::prepare_highlight_pipeline(){
}

void PdfViewQPainterWidget::prepare_non_compiled_line_drawing_pipeline(){
}
...
void PdfViewQPainterWidget::prepare_initial_render_pipeline(){
}

void PdfViewQPainterWidget::prepare_link_highlight_state(){
    std::array<float, 3> link_highlight_color = cc3(DEFAULT_LINK_HIGHLIGHT_COLOR);
    set_highlight_color(&link_highlight_color[0], 0.3f);
}

void PdfViewQPainterWidget::mouseMoveEvent(QMouseEvent* mouse_event) {

    if (handle_mouse_move_event(mouse_event)) update();
}

void PdfViewQPainterWidget::mousePressEvent(QMouseEvent* mevent) {

    if (handle_mouse_press_event(mevent)) update();
}

void PdfViewQPainterWidget::mouseReleaseEvent(QMouseEvent* mouse_event) {

    if (handle_mouse_release_event(mouse_event)) update();
}

void PdfViewQPainterWidget::wheelEvent(QWheelEvent* wevent) {
    if (handle_wheel_event(wevent)) update();
...
PdfViewQPainterWidget::~PdfViewQPainterWidget() {
}

PdfViewQPainterWidget::PdfViewQPainterWidget(DocumentView* document_view_, PdfRenderer* pdf_renderer_, DocumentManager* docman, bool is_helper_, QWidget* parent) :
    QWidget(parent)
{
    document_view = document_view_;
    pdf_renderer = pdf_renderer_;
...
QShader get_rhi_shader(const QString &file_path){
    ...
    qDebug() << "could not load shader " << file_path;
...
void PdfViewRhiWidget::initialize(QRhiCommandBuffer *command_buffer)
{
    ...
    { // always reset the qpainter texture because the window might have been resized
        ...
        qpainter_texture->create();

        qpainter_uniform_buffer.reset(rhi()->newBuffer(QRhiBuffer::Dynamic, QRhiBuffer::UniformBuffer, sizeof(float)));
        qpainter_uniform_buffer->create();
    ...
    if (!colored_rect_pipeline) {
        ...
        highlights_borders_vertex_buffer_ptr.reset(rhi_ptr->newBuffer(QRhiBuffer::Dynamic, QRhiBuffer::VertexBuffer, highlights_vertex_buffer_size));
        highlights_borders_vertex_buffer_ptr->create();
        ...
        //     { QRhiShaderStage::Vertex, get_rhi_shader(QLatin1String("/Volumes/my_external_ssd/sioyek/sioyek-private/qsb_shaders/prebuilt/test.vert.qsb")) },
        //     { QRhiShaderStage::Fragment, get_rhi_shader(QLatin1String("/Volumes/my_external_ssd/sioyek/sioyek-private/qsb_shaders/prebuilt/test.frag.qsb")) }
        // });
        ...
        QRhiVertexInputLayout highlight_borders_input_layout;
        highlight_borders_input_layout.setBindings({
            { 2 * sizeof(float) },
        });
        ...
        // QRhiVertexInputLayout test_rect_input_layout;
        // test_rect_input_layout.setBindings({
        //     { 2 * sizeof(float) },
        // });
        ...
        highlight_borders_input_layout.setAttributes({
            { 0, 0, QRhiVertexInputAttribute::Float2, 0 },
        });
        ...
        // test_rect_pipeline->setRenderPassDescriptor(renderTarget()->renderPassDescriptor());
        // test_rect_pipeline->create();
        ...
        std::vector<float> static_qpainter_vertices = {
            -1, -1,
            -1, 1,
            1, -1,
        ...
        resourceUpdates->uploadStaticBuffer(qpainter_vertex_buffer.get(), 0, static_qpainter_vertices.size() * sizeof(float), static_qpainter_vertices.data());
...
void PdfViewRhiWidget::render(QRhiCommandBuffer *command_buffer)
{
    ...
    update_resources_for_current_frame_drawing_calls(resource_updates);
...
void PdfViewRhiWidget::render_current_frame_textures(QRhiCommandBuffer* command_buffer){
    ...
    for (int i = 0; i < max_iter; i++){
        ...
        if (texture){
            ...
            if (current_frame_texture_render_calls[i].scissor_rect.has_value()){
                set_scissor_rect(command_buffer, current_frame_texture_render_calls[i].scissor_rect.value());
            }
            else{
...
void PdfViewRhiWidget::set_scissor_rect(QRhiCommandBuffer* command_buffer, NormalizedWindowRect scissor_rect){
    int x0 = (scissor_rect.x0 + 1) / 2 * width() * get_device_pixel_ratio();
    int x1 = (scissor_rect.x1 + 1) / 2 * width() * get_device_pixel_ratio();
    int y0 = (scissor_rect.y0 + 1) / 2 * height() * get_device_pixel_ratio();
    int y1 = (scissor_rect.y1 + 1) / 2 * height() * get_device_pixel_ratio();
    if (y1 < y0){
        std::swap(y1, y0);
    }
...
void PdfViewRhiWidget::set_overview_scissor(QRhiCommandBuffer* command_buffer, std::optional<OverviewState> overview){
    ...
    set_scissor_rect(command_buffer, overview_rect);
}
...
void PdfViewRhiWidget::update_resources_for_current_frame_highlight_render_calls(QRhiResourceUpdateBatch* update_batch){
    ...
    for (int i = 0; i < max_iter; i++){
        ...
        if (current_frame_highlight_rect_render_calls[i].flags & HighlightRenderFlags::HRF_BORDER || current_frame_highlight_rect_render_calls[i].flags & HighlightRenderFlags::HRF_UNDERLINE){
            ...
            int border_offset = 10 * sizeof(float) * i;
            update_batch->updateDynamicBuffer(highlights_borders_vertex_buffer_ptr.get(), border_offset, 10 * sizeof(float), border_vertices);
        }
    }
}
...
void PdfViewRhiWidget::render_texture(std::optional<SioyekTextureType> texture, NormalizedWindowRect rect, ColorPalette palette){
    if (texture.has_value()){
        ...
        render_call.scissor_rect = current_stencil;
...
void PdfViewRhiWidget::draw_stencil_rects(const std::vector<NormalizedWindowRect>& window_rects){}
void PdfViewRhiWidget::use_stencil_to_write(bool eq){}
void PdfViewRhiWidget::disable_stencil(){}
...
void PdfViewRhiWidget::set_stencil_for_two_page(int page, PagelessDocumentRect page_content, bool stencils_allowed, float zoom_level){
    if (BACKGROUND_PIXEL_FIX || (document_view->is_two_page_mode() && (stencils_allowed))) {
        ...
        page_drect.rect = page_content;
        current_stencil = page_drect.to_window_normalized(dv());
        // dv->docu
    }
}
...
void PdfViewRhiWidget::bind_default(){}
void PdfViewRhiWidget::bind_vertex_array(){}

void PdfViewRhiWidget::render_original_color_images(int page_number, std::optional<OverviewState> overview, ColorPalette forced_color_palette, bool stencils_allowed){}

bool PdfViewRhiWidget::is_opengl() {
    return false;
}

QWidget* PdfViewRhiWidget::get_widget(){
    return this;
}
...
void PdfViewRhiWidget::delete_old_texture_shader_resource_bindings(){
    ...
    if (texture_shader_resource_bindings.size() > RESOURCE_BINDINGS_THRESHOLD){
        ...
        for (int i = 0; i < num_to_delete; i++){
            ...
            delete texture_shader_resource_bindings[index].uniform_buffer;
...
SioyekTextureShaderResourceBinding* PdfViewRhiWidget::get_shader_resource_binding_for_texture(QRhiTexture* texture, bool is_icon){
    ...
    result.uniform_buffer = uniform_buffer;
